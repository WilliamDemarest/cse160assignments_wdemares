...
let t_triangleBufferNormal = null;

function init_triangleBuffer(FSIZE){
  t_triangleBuffer = gl.createBuffer();
  if (!t_triangleBuffer) {
    console.log('Failed to create the buffer object');
    return -1;
  }
...
function init_triangleBufferNormal(FSIZE){
  t_triangleBufferNormal = gl.createBuffer();
  if (!t_triangleBufferNormal) {
    console.log('Failed to create the buffer object');
    return -1;
  }
  // Bind the buffer object to target
  gl.bindBuffer(gl.ARRAY_BUFFER, t_triangleBufferNormal);
...
function drawAllTriangle3DUVNormal(data) {

  let vertices = new Float32Array(data);
  FSIZE = vertices.BYTES_PER_ELEMENT;

  var n = 3+2+3; // The number of data in vertex

  if (t_triangleBufferNormal == null){
    init_triangleBufferNormal(FSIZE);
  }
